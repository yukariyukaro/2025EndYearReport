      ...
      <div className={styles.container} style={{ backgroundImage: 'url("imgs/page19/background.png")' }}>
        <div className={styles.topArea}>
          {hasData ? (
            ...
            <div className={`hide page19-reveal-2 ${styles.textRow}`}>
              <div className={styles.mainTitle}>{pageData?.social_description ?? "暂无私信互动记录"}</div>
            </div>
          )}
        </div>
        ...
        <div className={styles.contentArea}>
          {hasData ? (
            ...
            <div className={`hide page19-reveal-5 ${styles.descriptionText}`}>
              {pageData?.activity_description ?? "今年还没有使用私信功能，\n下次遇见想聊的人，不妨主动打个招呼~"}
            </div>
          )}
        </div>
